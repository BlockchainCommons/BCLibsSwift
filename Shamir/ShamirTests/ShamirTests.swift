...
extension Shamir.Share: CustomStringConvertible {
...
class ShamirTests: XCTestCase {
    func testIdentify() {
        XCTAssertEqual(Shamir.identify(), "Shamir")
    }

    func test1() throws {
        let secret = "00112233445566778899aabbccddeeff".hexData
        let shares = Shamir.splitSecret(threshold: 2, shareCount: 3, secret: secret, randomGenerator: fakeRandom)
        ...
        let recoveredSecret = try Shamir.recoverSecret(shares: recoveredShares)
        XCTAssertEqual(secret, recoveredSecret)

        recoveredShares.removeFirst()
        let badRecoveredSecret = try Shamir.recoverSecret(shares: recoveredShares)
